...
@Composable
fun MovementScreen(navController: NavController, mainViewModel: MainViewModel, userInfo: UserInfo, task: StorekeeperTask) {
    val operationViewModel = hiltViewModel<OperationalViewModel>()
    operationViewModel.user = userInfo
    operationViewModel.task = task
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            Row(
                ...
                Column(
                    ...
                    TitleText(text = stringResource(id = R.string.movement))
                    SubtitleText(text = task.number)
                }
            }
            Row(
                modifier = Modifier
                ...
                var packs = 0
                operationViewModel.task?.let { task->
                    task.ProductToMove.map { product->
                    packs += product.packsQuantity
                    }
                    Text(
                        text = "${stringResource(id = R.string.task)} $packs пал.",
                        modifier = Modifier
                            .weight(1F)
                            .clickable {
                    ...
                    Text(
                        text = "${stringResource(id = R.string.fact)} ${operationViewModel.fullPalletsList.size} пал.",
                        modifier = Modifier
                            .weight(1F)
                            .clickable {
                                setChecked(!isChecked)
            ...
            LazyColumn(
                ...
                if (!isChecked) {
                    operationViewModel.task?.let{
                        items(it.ProductToMove) { product ->
                            ProductToMoveItem(product = product)
                        }
                    }
                } else {
                    val groupedPalletsList = operationViewModel.fullPalletsList.groupBy { it.cellFrom!!.cellName }
                            ...
                            SwipeToDismiss(state = dismissState,
                                ...
                                dismissContent = {
                                    Column(modifier = Modifier
                                        .fillMaxWidth()
                                        .clickable {
                                            operationViewModel.readScannedData(barcode = pData.palletBarcode)
                                        }
                                        .background(Color.White)
                                    ) {
                                        PalletMoved(pallet = pData)
                                        /*Text(
                ...
                when {
                    ...
                    it.startsWith("08") -> {
                        if (operationViewModel.aCell != null) {
                            ...
                            setChecked(true)
                            coroutineScope.launch { listState.animateScrollToItem(index = operationViewModel.fullPalletsList.size) }
                        } else {
